...
public class TIPDistance extends TIPHeader {
  ...
  public void buildResponse() {
    ...
    numerator += Math.pow( Math.cos(origin_lat) * Math.sin(dest_lat) - Math.sin(origin_lat) * Math.cos(dest_lat) * Math.cos(delta_lon), 2 );
    numerator  = Math.sqrt(numerator);

    double denominator = Math.sin(origin_lat) * Math.sin(dest_lat) + Math.cos(origin_lat) * Math.cos(dest_lat) * Math.cos(delta_lon);
